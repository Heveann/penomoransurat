...
<div class="container-fluid">
    ...
    <div class="form-page-header">
        ...
        <h5 class="form-page-title">Permintaan Nomor</h5>
        <p class="form-page-subtitle">Isi formulir di bawah untuk mengajukan nomor surat baru</p>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <span class="form-card-header-title">Form Permintaan Surat</span>
        </div>
                        ...
                        <div>
                            ...
                            <select class="input-clean" id="jenis_naskah" name="jenis_naskah" required>
                                ...
                                @foreach($jenisNaskah as $jenis)
                                    <option value="{{ $jenis->nama }}" {{ old('jenis_naskah') == $jenis->nama ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-grid" style="margin-top: 1.25rem;">
                        <div>
                            ...
                            <select name="pengolah" id="pengolah" class="input-clean" required>
                                ...
                                @foreach($unitKerjas as $unit)
                                    <option value="{{ $unit->nama }}" {{ old('pengolah') == $unit->nama ? 'selected' : '' }}>
                                        {{ $unit->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="sifat_naskah" id="sifat_naskah" class="input-clean">
                                <option value="" selected disabled>Pilih Sifat Naskah</option>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        <div>
                            ...
                            <input type="date" class="input-clean" name="tanggal_ditetapkan" id="tanggal_ditetapkan" value="{{ old('tanggal_ditetapkan', date('Y-m-d')) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" class="input-clean" name="tanggal_berlaku" value="{{ old('tanggal_berlaku') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid single">
                        <div>
                            ...
                            <textarea class="input-clean" name="hal" rows="4" placeholder="Masukkan hal/perihal surat..." required>{{ old('hal') }}</textarea>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    // Initialize Select2 for Pengolah (Unit Kerja) - dropdownParent ensures it always opens downward
    $('#pengolah').select2({
    ...
    $('#form-surat').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(res) {
                var nomorSurat = res.nomor_surat ? '<br><b>Nomor Surat: ' + res.nomor_surat + '</b>' : '';

                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil!',
                    html: 'Permintaan berhasil disimpan.' + nomorSurat,
                    confirmButtonText: 'OK',
                    confirmButtonColor: '#6366f1',
                    allowOutsideClick: false
                }).then(function() {
                    window.location.href = "{{ route('user.dashboard-user') }}";
                });
            },
            error: function(xhr) {
                ...
                let errorMessage = 'Gagal menyimpan permintaan. Pastikan semua data sudah benar.';
